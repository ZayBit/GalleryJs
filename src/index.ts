...
class gallery {
    ...
    private config_gallery: config;
    constructor(class_name: string, config: config) {
    ...
    init() {
        ...
        const container = document.querySelector(this.class_name),
        ...
        for (let i = 0; i < img_length; i++) {
            img[i].addEventListener('click', function () {
                let current_image = this,
                    clone = current_image.cloneNode();
                // create lightbox and controls
                ...
                function direction(btn) {
                    (img[main_index] as HTMLElement).style.opacity = '1';
                    (img[index] as HTMLElement).style.opacity = '1';
                ...
                const arrowEvent = resizeParams => {
                    currentCount.innerHTML = `${index + 1} / ${img_length}`;
                    let el_img = img[index] as HTMLImageElement;
    ...
    getParams(element) {
        let boundin = element.getBoundingClientRect(),
            boundin_x = boundin.x,
            boundin_y = boundin.y,
            boundin_width = boundin.width,
            boundin_height = boundin.height;
    ...
    resize(resizeParams) {
        ...
        let naturalWidth = current_image.naturalWidth,
            naturalHeight = current_image.naturalHeight;
